...
class UserController {
    async registration(request, response, next) {
        ...
        try {
            ...
            const jwtHash = jwt.sign({ id: user.id, name: user.name, role: user.role }, process.env.SECRET, { expiresIn: "24h" });
    ...
    async checkAuth(request, response, next) {
        const jwtHash = jwt.sign({ id: request.user.id, name: request.user.name, role: request.user.role }, process.env.SECRET, { expiresIn: "24h" });
        return response.json(jwtHash);
    }
}
